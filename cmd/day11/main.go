package main

import (
	"bytes"
	. "github.com/asuahsahua/advent2019/cmd/common"
	"github.com/asuahsahua/advent2019/cmd/intcode"
)

// --- Day 11: Space Police ---
// On the way to Jupiter, you're pulled over by the Space Police.

// "Attention, unmarked spacecraft! You are in violation of Space Law! All
// spacecraft must have a clearly visible registration identifier! You have
// 24 hours to comply or be sent to Space Jail!"
type Hull struct{
	Panels map[Point2D]int64
}

func NewHull() *Hull {
	return &Hull{
		Panels: make(map[Point2D]int64),
	}
}

// (this will help us later to view the hull)
func (h *Hull) ToString() string {
	whitePanels := make([]Point2D, 0)
	for point, color := range h.Panels {
		if color == WHITE {
			whitePanels = append(whitePanels, point)
		}
	}

	box := ResolveBoundingBox(whitePanels)
	var output bytes.Buffer

	for x := box.MinX; x <= box.MaxX; x++ {
		for y := box.MinY; y <= box.MaxY; y++ {
			if h.Panels[Point2D{x, y}] == WHITE {
				output.WriteByte('#')
			} else {
				output.WriteByte(' ')
			}
		}
		output.WriteByte('\n')
	}

	return output.String()
}

// Not wanting to be sent to Space Jail, you radio back to the Elves on
// Earth for help. Although it takes almost three hours for their reply
// signal to reach you, they send instructions for how to power up the
// emergency hull painting robot.
var BrainCode = `3,8,1005,8,337,1106,0,11,0,0,0,104,1,104,0,3,8,1002,8,-1,10,101,1,10,10,4,10,108,1,8,10,4,10,1002,8,1,28,2,1,15,10,2,2,10,10,1,1107,0,10,2,1105,18,10,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,1,10,4,10,101,0,8,67,1,1003,4,10,2,1007,14,10,1006,0,64,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,0,10,4,10,102,1,8,100,2,102,15,10,3,8,1002,8,-1,10,1001,10,1,10,4,10,108,0,8,10,4,10,1001,8,0,125,2,1003,7,10,1006,0,10,2,1007,13,10,2,103,14,10,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,1,10,4,10,101,0,8,163,1006,0,5,3,8,1002,8,-1,10,1001,10,1,10,4,10,1008,8,0,10,4,10,102,1,8,188,1,1101,2,10,1006,0,82,3,8,1002,8,-1,10,101,1,10,10,4,10,1008,8,0,10,4,10,101,0,8,217,1,1109,1,10,1,109,9,10,1,1009,9,10,1006,0,41,3,8,102,-1,8,10,1001,10,1,10,4,10,1008,8,1,10,4,10,102,1,8,254,2,104,1,10,2,8,15,10,3,8,1002,8,-1,10,1001,10,1,10,4,10,1008,8,1,10,4,10,101,0,8,284,1,1107,11,10,3,8,102,-1,8,10,101,1,10,10,4,10,108,0,8,10,4,10,101,0,8,309,2,1001,10,10,1006,0,49,101,1,9,9,1007,9,1058,10,1005,10,15,99,109,659,104,0,104,1,21101,937267929896,0,1,21101,0,354,0,1106,0,458,21102,1,936995566336,1,21102,1,365,0,1106,0,458,3,10,104,0,104,1,3,10,104,0,104,0,3,10,104,0,104,1,3,10,104,0,104,1,3,10,104,0,104,0,3,10,104,0,104,1,21101,3263269979,0,1,21101,0,412,0,1106,0,458,21102,1,46174071899,1,21101,0,423,0,1106,0,458,3,10,104,0,104,0,3,10,104,0,104,0,21101,825544561428,0,1,21102,446,1,0,1105,1,458,21102,1,867966018404,1,21101,457,0,0,1106,0,458,99,109,2,21202,-1,1,1,21102,40,1,2,21101,489,0,3,21102,1,479,0,1105,1,522,109,-2,2106,0,0,0,1,0,0,1,109,2,3,10,204,-1,1001,484,485,500,4,0,1001,484,1,484,108,4,484,10,1006,10,516,1102,0,1,484,109,-2,2105,1,0,0,109,4,2102,1,-1,521,1207,-3,0,10,1006,10,539,21101,0,0,-3,21201,-3,0,1,22101,0,-2,2,21101,1,0,3,21102,558,1,0,1105,1,563,109,-4,2105,1,0,109,5,1207,-3,1,10,1006,10,586,2207,-4,-2,10,1006,10,586,22102,1,-4,-4,1106,0,654,22101,0,-4,1,21201,-3,-1,2,21202,-2,2,3,21102,1,605,0,1105,1,563,22101,0,1,-4,21102,1,1,-1,2207,-4,-2,10,1006,10,624,21102,1,0,-1,22202,-2,-1,-2,2107,0,-3,10,1006,10,646,21201,-1,0,1,21102,646,1,0,106,0,521,21202,-2,-1,-2,22201,-4,-2,-4,109,-5,2106,0,0`

// There's just one problem: you don't have an emergency hull painting
// robot. You'll need to build a new emergency hull painting robot.
type Robot struct{
	Brain *intcode.IntcodeMachine
	Position Point2D
	Direction int
}

// (defining these as clockwise so we can just -/+1 mod 4 when moving 90 degress!)
const (
	NORTH = 0
	EAST = 1
	SOUTH = 2
	WEST = 3
)

func NewRobot() *Robot {
	return &Robot{
		// The Intcode program will serve as the brain of the robot. 
		Brain: intcode.NewIntcodeMachineStr(BrainCode),
		// (choosing origin as an arbitrary starting position)
		Position: Point2D{0, 0},
		// The robot starts facing up.
		Direction: NORTH,
	}
}

// The robot needs to be able to move around on the grid of square panels on the
// side of your ship, detect the color of its current panel, and paint its
// current panel black or white. (All of the panels are currently black.)
const (
	BLACK = int64(0)
	WHITE = int64(1)
)

const (
	LEFT = 0
	RIGHT = 1
)

func (r *Robot) RunPaintingProgram(hull *Hull) {
	// (start that async robot!)
	go r.Brain.Run()

	for {
		// The program uses input instructions to access the robot's camera
		// (maps use the default value for the structure if not present, 0=black in our case - perfect!)
		r.Brain.Input <- hull.Panels[r.Position]

		// Then, the program will output two values:
		// 1. the color to paint the panel (0=black 1=white)
		outputColor, chanOpen := <- r.Brain.Output
		// (if we received the HCF signal, then we're done)
		if !chanOpen {
			break
		}
		hull.Panels[r.Position] = outputColor

		// 2. direction the robot should turn (0=left 1=right)
		turnDirection := <- r.Brain.Output
		switch turnDirection {
		case LEFT:
			r.Direction = (r.Direction + 3) % 4
		case RIGHT:
			r.Direction = (r.Direction + 1) % 4
		default:
			Panic("Unexpected turn direction %d", turnDirection)
		}

		// After the robot turns, it should always move forward exactly one panel
		switch r.Direction {
		case NORTH:
			r.Position.Y++
		case SOUTH:
			r.Position.Y--
		case EAST:
			r.Position.X++
		case WEST:
			r.Position.X--
		default:
			Panic("Unexpected direction %d", r.Direction)
		}
	}
}

func main() {
	robot := NewRobot()
	// Before you deploy the robot, you should probably have an estimate of the
	// area it will cover: specifically, you need to know the number of panels
	// it paints at least once, regardless of color.
	hull := NewHull()
	robot.RunPaintingProgram(hull)
	// (only painted panels will be present, otherwise the default black is used)
	Part1("%d", len(hull.Panels))

	// --- Part Two ---
	// You're not sure what it's trying to paint, but it's definitely not a
	// registration identifier. The Space Police are getting impatient.
	
	// Checking your external ship cameras again, you notice a white panel
	// marked "emergency hull painting robot starting panel". The rest of the
	// panels are still black, but it looks like the robot was expecting to
	// start on a white panel, not a black one.
	hull2 := NewHull()
	hull2.Panels[Point2D{0,0}] = WHITE
	
	// Based on the Space Law Space Brochure that the Space Police attached to
	// one of your windows, a valid registration identifier is always eight
	// capital letters. After starting the robot on a single white panel
	// instead, what registration identifier does it paint on your hull?
	robot2 := NewRobot()
	robot2.RunPaintingProgram(hull2)
	Part2("\n%s", hull2.ToString())
}
